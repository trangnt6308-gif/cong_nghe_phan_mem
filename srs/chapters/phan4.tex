...
Hệ thống \texttt{SmartDroneDelivery} được xây dựng theo mô hình kiến trúc đa tầng nhằm tách biệt các trách nhiệm và nâng cao khả năng bảo trì, nâng cấp phần mềm. Kiến trúc hệ thống bao gồm ba tầng chính như sau:

\begin{enumerate}[leftmargin=*]
  \item \textbf{Tầng Giao Diện:}
    \begin{itemize}
      \item \textbf{Customer Web Portal:} Ứng dụng Web dành cho Khách hàng giao tiếp qua trình duyệt để gửi yêu cầu đặt hàng mới, nhận thông báo điện tử, định vị hành trình bay của drone thời gian thực và trò chuyện trực tiếp với Trợ lý ảo chatbot.
      \item \textbf{Management Web Portal:} Ứng dụng Web quản lý dành cho nhà nội bộ gồm Quản lý Logistics, Điều phối viên và Nhân viên vận hành trạm. Hỗ trợ duyệt hàng, lập lịch bay, quản lý vận hành trạm và thiết bị drone, xử lý sự cố phát sinh.
      \item Hai ứng dụng Web trên liên kết với tầng Backend thông qua giao thức HTTP/HTTPS hỗ trợ REST API và WebSockets để truyền tọa độ trực tuyến.
    \end{itemize}
  \item \textbf{Tầng Nghiệp Vụ:}
    \begin{itemize}
      \item Được phát triển dựa trên ngôn ngữ \textbf{Python} và khung ứng dụng \textbf{Flask}.
      \item Flask tiếp nhận các yêu cầu REST API, thực hiện kiểm lỗi, và kiểm soát các ràng buộc nghiệp vụ như giới hạn tải trọng 5kg, chặn sửa đổi đơn sau khi duyệt.
      \item Tầng Backend còn tích hợp dịch vụ phụ trợ như \textbf{LLM API} thông qua giao thức HTTPS để vận hành trợ lý chatbot AI đàm thoại với khách hàng.
    \end{itemize}

    \begin{figure}[H]
      \centering
      \includegraphics[width=0.9\textwidth]{so-do/diagrams/sodokientruc/sơ đồ kiến trúc BE.pdf}
  ...
  \item \textbf{Tầng Dữ Liệu:}
    \begin{itemize}
      ...
      \item Mối liên kết giữa \textbf{Flask Backend} và \textbf{PostgreSQL Database} được thực hiện thông qua kết nối mạng \textbf{TCP/IP} bằng thư viện SQLAlchemy ORM trên cổng kết nối mặc định \texttt{5432}. Tất cả các truy vấn dữ liệu và giao dịch được mã hóa bảo mật thông qua giao thức TCP.
    \end{itemize}
\end{enumerate}
...
Hệ thống SmartDroneDelivery bao gồm hai nhóm package chính: giao diện người dùng ở phía Frontend và tầng xử lý nghiệp vụ ở phía Backend.

\subsubsection{Sơ Đồ Package Frontend}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{so-do/diagrams/SoDoPackage_FE.png}
  \caption{Sơ đồ package Frontend của hệ thống SmartDroneDelivery}
  \label{fig:package-fe}
\end{figure}

\subsubsection{Sơ Đồ Package Backend}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{so-do/diagrams/SoDoPackage_BE.png}
  \caption{Sơ đồ package Backend của hệ thống SmartDroneDelivery}
  \label{fig:package-be}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/database_schema.pdf}
  \caption{Sơ đồ lược đồ cơ sở dữ liệu vật lý của hệ thống SmartDroneDelivery}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|l|l|>{\raggedright\arraybackslash}X|}
    ...
    ten\_vai\_tro        & VARCHAR(50)     & NOT NULL, UNIQUE & Tên vai trò như Admin, Dispatcher, v.v. \\ \hline
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|l|l|>{\raggedright\arraybackslash}X|}
    ...
    gioi\_tinh           & VARCHAR(10)     & CHECK           & Giới tính gồm Nam, Nữ hoặc Khác \\ \hline
    so\_dien\_thoai      & VARCHAR(20)     & NULL            & Số điện thoại liên hệ \\ \hline
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|l|l|>{\raggedright\arraybackslash}X|}
    ...
    dia\_chi\_cu\_the    & TEXT            & NOT NULL        & Địa chỉ chi tiết gồm số nhà và tên đường \\ \hline
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|l|l|>{\raggedright\arraybackslash}X|}
    ...
    cong\_suat\_pin      & INT             & NOT NULL, CHECK & Dung lượng pin còn lại, từ 0 đến 100\% \\ \hline
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_login.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_create_order.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_update_cancel_order.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_approve_order.pdf}
  \caption{Sơ đồ tuần tự chức năng Phê duyệt yêu cầu giao hàng}
  \label{fig:seq-uc04}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_schedule_delivery.pdf}
  \caption{Sơ đồ tuần tự chức năng Lập lịch giao hàng}
  \label{fig:seq-uc05}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|c|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{4.5cm}|>{\raggedright\arraybackslash}X|}
    ...
    5 & \texttt{Yeu\allowbreak Cau\allowbreak Su\allowbreak Co} & \texttt{ly\allowbreak Do}, \texttt{muc\allowbreak Do\allowbreak Nghiem\allowbreak Trong} & DTO chứa chi tiết báo cáo sự cố do thời tiết, pin hỏng, v.v. \\ \hline
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_fail_delivery.pdf}
  \caption{Sơ đồ tuần tự chức năng Xử lý giao hàng thất bại}
  \label{fig:seq-uc06}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_station_check.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_tracking_realtime.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_success_delivery.pdf}
...
\begin{figure}[H]
  ...
  \caption{Sơ đồ lớp chức năng Ước lượng thời gian giao hàng dự kiến}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|c|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{4.5cm}|>{\raggedright\arraybackslash}X|}
    ...
    \textbf{STT} & \textbf{Tên lớp} & \textbf{Thuộc tính / Phương thức} & \textbf{Mô tả} \\
    \hline
    1 & \texttt{Dieu\allowbreak Khien\allowbreak Uoc\allowbreak Luong} & \texttt{lay\allowbreak Thoi\allowbreak Gian\allowbreak Du\allowbreak Kien\allowbreak (ma\allowbreak Don\allowbreak Hang)} & REST API controller tiếp nhận yêu cầu tính toán thời gian đến dự kiến. \\ \hline
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_eta_estimation.pdf}
  \caption{Sơ đồ tuần tự chức năng Ước lượng thời gian giao hàng dự kiến}
  \label{fig:seq-uc10}
\end{figure}

\subsection{Trợ Lý AI Cho Khách Hàng}

\subsubsection{Sơ Đồ Lớp}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{diagrams/sodolop/UC11_ChatbotAI.png}
  \caption{Sơ đồ lớp chức năng Trợ lý AI cho khách hàng}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|c|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{4.5cm}|>{\raggedright\arraybackslash}X|}
    ...
    5 & \texttt{Khach\allowbreak Hang\allowbreak Mo\allowbreak Hinh\allowbreak Ngon\allowbreak Ngu} & \texttt{tao\allowbreak Phan\allowbreak Hoi\allowbreak (loi\allowbreak Nhac)} & Interface giao tiếp với mô hình ngôn ngữ lớn bên ngoài như Gemini hoặc Groq API. \\ \hline
    ...
    \hline
  \end{tabularx}
  \caption{Đặc tả các lớp chức năng Trợ Lý AI Cho Khách Hàng}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_chatbot.pdf}
  \caption{Sơ đồ tuần tự chức năng Trợ lý AI cho khách hàng}
  \label{fig:seq-uc11}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_manage_user.pdf}
  \caption{Sơ đồ tuần tự chức năng Tạo và quản lý tài khoản người dùng}
  \label{fig:seq-uc12}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{so-do/sodo-tuantu/seq_manage_drone.pdf}
