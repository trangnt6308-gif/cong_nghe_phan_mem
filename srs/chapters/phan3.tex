...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|X|l|}
    ...
    UC-15 & Xem báo cáo và thống kê (Dashboard)      & Logistics Manager, Admin \\ \hline
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    \textbf{Luồng sự kiện chính} &
      \begin{enumerate}[leftmargin=*, nosep, topsep=0pt]
        ...
        \item Nếu Admin muốn sửa thông tin: chọn thiết bị, cập nhật trạng thái hoạt động (Sẵn sàng/Bảo trì/Hỏng) hoặc cập nhật dung lượng pin $\rightarrow$ Hệ thống lưu thay đổi.
        \item Nếu Admin muốn xóa: chọn thiết bị $\rightarrow$ Hệ thống xóa bản ghi hoặc đổi trạng thái ngừng hoạt động.
      \end{enumerate} \\ \hline
    \textbf{Luồng rẽ nhánh / Ngoại lệ} &
      \textbf{3a/4a.} Trạng thái hoặc dung lượng pin nhập không hợp lệ (ví dụ pin ngoài khoảng 0-100) $\rightarrow$ Hệ thống báo lỗi, yêu cầu chỉnh sửa lại.
      \newline
      \textbf{5a.} Drone đang thực hiện chuyến bay giao hàng (trạng thái ``Đang giao'') $\rightarrow$ Hệ thống cảnh báo không được xóa hoặc đổi sang trạng thái bảo trì/hỏng. \\ \hline
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    \hline
    \textbf{Tác nhân}           & Quản trị viên (Admin), Quản lý logistics \\ \hline
    ...
    \textbf{Luồng sự kiện chính} &
      \begin{enumerate}[leftmargin=*, nosep, topsep=0pt]
        ...
        \item Nhập hoặc cập nhật thông tin trạm (tên trạm, địa chỉ, tọa độ lat/lng và công suất tối đa).
      ...
      \end{enumerate} \\ \hline
    \textbf{Luồng rẽ nhánh / Ngoại lệ} &
      \textbf{4a.} Trạm đang có đơn hàng chờ (Đang giao hoặc Đã lên lịch) $\rightarrow$ Hệ thống cảnh báo không cho chuyển trạng thái sang Bảo trì/Ngừng trừ khi gán lại đơn hàng. \\ \hline
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    \hline
    \multicolumn{2}{|l|}{\textbf{Xem báo cáo và thống kê (Dashboard)}} \\
    \hline
    ...
    \textbf{Luồng sự kiện chính} &
      \begin{enumerate}[leftmargin=*, nosep, topsep=0pt]
        \item Người dùng chọn mục ``Bảng điều khiển'' (Dashboard).
        \item Hệ thống kết xuất dữ liệu và hiển thị các chỉ số cốt lõi (tổng đơn hàng, tỉ lệ thành công, hiệu suất Drone).
        \item Người dùng lọc dữ liệu theo khoảng thời gian (ngày, tuần, tháng).
    ...
    \hline
  \end{tabularx}
  \caption{Đặc tả Xem báo cáo và thống kê (Dashboard)}
  \label{tab:uc15}
\end{table}
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    \textbf{Luồng sự kiện chính} &
      \begin{enumerate}[leftmargin=*, nosep, topsep=0pt]
        ...
        \item Hệ thống lấy dữ liệu thô (thời gian bay, trạm đi/đến, trạng thái sự cố nếu có).
...
\begin{itemize}
  ...
  \item Xóa trạm.
  \item Cập nhật trạng thái hoạt động (Đang hoạt động, Bảo trì hoặc Ngừng).
\end{itemize}
...
\begin{itemize}
  ...
  \item Cập nhật thông tin drone (trạng thái hoạt động, dung lượng pin, ngày bảo trì).
  \item Xóa hoặc vô hiệu hóa thiết bị drone.
  \item Giám sát danh sách drone và mức năng lượng pin thời gian thực.
\end{itemize}

\subsubsection{FR-10: Thống kê và báo cáo giao hàng (Dashboard)}
...
Để làm rõ hơn phạm vi quyền hạn và các chức năng hệ thống phân phối cho từng nhóm người dùng, dưới đây là ma trận phân quyền sử dụng chức năng (Use Case) chi tiết:
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|X|c|c|c|c|c|}
    ...
    UC-15 & Xem báo cáo và thống kê (Dashboard)      &            &            &            & \checkmark & \checkmark \\ \hline
  ...
  - \textbf{C}: Khách hàng (Customer) \\
  - \textbf{SO}: Nhân viên vận hành trạm (Station Operator) \\
  - \textbf{D}: Điều phối viên (Dispatcher) \\
  - \textbf{LM}: Quản lý logistics (Logistics Manager) \\
  - \textbf{A}: Quản trị viên hệ thống (Admin) \\
  - \textbf{\checkmark}: Biểu thị vai trò có quyền thao tác thực hiện Use Case.}
  \end{spacing}
  \caption{Ma trận phân quyền sử dụng chức năng (Use Case) hệ thống SmartDroneDelivery}
...
\begin{itemize}
  ...
  \item API phải có tài liệu hướng dẫn và đặc tả đầy đủ (Swagger hoặc OpenAPI).
\end{itemize}
...
\begin{itemize}
  ...
  \item Ghi nhật ký kiểm toán chi tiết cho mọi thao tác quan trọng trên hệ thống.
...
\begin{itemize}
  ...
  \item Hệ thống phải có cơ chế ghi log mọi thao tác quan trọng (tạo, sửa hoặc xóa đơn, thay đổi trạng thái kiện hàng, quản lý tài khoản).
